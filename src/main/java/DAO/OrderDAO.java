...
public class OrderDAO {
	...
	public boolean transferOrder(int product_id, String order_code) {
		boolean flag = false;
		String query = "UPDATE orders SET status = 1 WHERE product_id = ? AND order_code = ?";
		try {
			pst = con.prepareStatement(query);
			pst.setInt(1, product_id);
			pst.setString(2, order_code);
...
//		public List<Order> getByUser(int id)
		...
		public List<String> getDistinctDuplicatedOrderCodes(int user_id) {
		    List<String> distinctDuplicatedOrderCodes = new ArrayList<>();
		    String query = "SELECT DISTINCT order_code "
		                 + "FROM orders "
		    ...
		    try {
		        ...
		        while (rs.next()) {
		            ...
		            distinctDuplicatedOrderCodes.add(orderCode);
		        }
		        rs.close();
		    } catch (SQLException e) {
		        e.printStackTrace();
		    }
		    return distinctDuplicatedOrderCodes;
		}
